...
4. After each phase, the judge appends a log to `histories/YYYYMMDD_<phase>.md` using `histories/README.md`, and appends metrics to the single rolling log at `metrics/log.md` using `metrics/README.md` with scores on a 0-100 scale (100 is perfect). Keep `prompts/review_requirements.md` up to date with any standing review guardrails you want included in the prompt refresh.
