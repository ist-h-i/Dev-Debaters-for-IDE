...
3. At session start, have the orchestration role declare "Phase 1: Hearing start". Automation respects `.env`: when `AUTO_MODE=true`, orchestration invokes subsequent phases automatically; when `AUTO_MODE=false`, orchestration must ask the user before advancing to each new phase after Hearing. To auto-run from startup, send the block in `docs/codex_auto_start.md` with `AUTO_MODE=true`.
