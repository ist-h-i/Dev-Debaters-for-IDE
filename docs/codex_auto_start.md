...
4) Do not ask the user anything after hearing. At each phase end, call the judge and have them append directly: `histories/YYYYMMDDHHmmss_<phase>.md` using `histories/README.md`, and the phase-specific `metrics/<phase>.md` using `metrics/README.md` (never create dated metrics files or a combined cross-phase metrics log). Judges must not dump log content to chat; only confirm completion.
