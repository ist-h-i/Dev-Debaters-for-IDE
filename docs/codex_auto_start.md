...
4) Do not ask the user anything after hearing. At each phase end, call the judge and have them append directly: `histories/YYYYMMDDHHmmss_<phase>.md` using `histories/README.md`, and the shared `metrics/log.md` using `metrics/README.md` (never create dated metrics files). Judges must not dump log content to chat; only confirm completion.
